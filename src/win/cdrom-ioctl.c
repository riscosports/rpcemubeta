...
static void
ioctl_playaudio(uint32_t pos, uint32_t len)
{
	uint32_t addr;
...
static uint8_t
ioctl_getcurrentsubchannel(uint8_t *b, int msf)
{
	...
	if (msf) {
		...
		b[pos++] = (uint8_t) (temp >> 24);
		b[pos++] = (uint8_t) (temp >> 16);
		b[pos++] = (uint8_t) (temp >> 8);
		b[pos++] = (uint8_t) temp;
...
static void
ioctl_readsector(uint8_t *b, int sector)
{
	LARGE_INTEGER pos;
	unsigned long size;
...
static int
ioctl_readtoc(uint8_t *b, uint8_t start_track, int msf)
{
	int len = 4;
	unsigned long size;
	int c, d;
	...
	for (c = 0; c <= toc.LastTrack; c++) {
		if (toc.TrackData[c].TrackNumber >= start_track) {
			d = c;
			break;
		}
	}
	...
	for (c = d; c <= toc.LastTrack; c++) {
		...
		if (msf) {
			...
			b[len++] = (uint8_t) (temp >> 24);
			b[len++] = (uint8_t) (temp >> 16);
			b[len++] = (uint8_t) (temp >> 8);
			b[len++] = (uint8_t) temp;
		}
	}
	b[0] = (uint8_t) ((len - 4) >> 8);
...
static const ATAPI ioctl_atapi = {
	ioctl_ready,
	ioctl_readtoc,
