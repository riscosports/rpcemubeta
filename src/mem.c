...
mem_phys_read32(uint32_t addr)
{
	...
	case 0x03000000: /* IO */
		if ((addr & 0xc00000) == 0) {
			...
			case 4:
				...
				return podules_read16((addr >> 14) & 3, 0, addr & 0x3fff);
			case 7:
				/* Podule space 4, 5, 6, 7 */
				return podules_read16(((addr >> 14) & 3) + 4, 0, addr & 0x3fff);
			}
		}
	...
	case 0x0e000000:
	case 0x0f000000:
		return podules_read32((addr >> 24) & 7, 1, addr & 0xffffff);
...
mem_phys_read8(uint32_t addr)
{
	...
	case 0x03000000: /* IO */
		if ((addr & 0xc00000) == 0) {
			...
			case 4:
				/* Podule space 0, 1, 2, 3 */
				return podules_read8((addr >> 14) & 3, 0, addr & 0x3fff);
			case 7:
				/* Podule space 4, 5, 6, 7 */
				return podules_read8(((addr >> 14) & 3) + 4, 0, addr & 0x3fff);
			}
		}
	...
	case 0x0e000000:
	case 0x0f000000:
		return podules_read8((addr >> 24) & 7, 1, addr & 0xffffff);
...
mem_phys_write32(uint32_t addr, uint32_t val)
{
	...
	case 0x03000000: /* IO */
		if ((addr & 0xc00000) == 0) {
			...
			case 4:
				/* Podule space 0, 1, 2, 3 */
				podules_write16((addr >> 14) & 3, 0, addr & 0x3fff, val >> 16);
				break;
			case 7:
				/* Podule space 4, 5, 6, 7 */
				podules_write16(((addr >> 14) & 3) + 4, 0, addr & 0x3fff, val >> 16);
				break;
			}
		}
	...
	case 0x0e000000:
	case 0x0f000000:
		podules_write32((addr >> 24) & 7, 1, addr & 0xffffff, val);
		return;
...
mem_phys_write8(uint32_t addr, uint8_t val)
{
	...
	case 0x03000000: /* IO */
		if ((addr & 0xc00000) == 0) {
			...
			case 4:
				/* Podule space 0, 1, 2, 3 */
				podules_write8((addr >> 14) & 3, 0, addr & 0x3fff, val);
				break;
			case 7:
				/* Podule space 4, 5, 6, 7 */
				podules_write8(((addr >> 14) & 3) + 4, 0, addr & 0x3fff, val);
				break;
			}
		}
		if ((machine.model == Model_Phoebe) && (addr & 0xcff000) == 0x800000) {
	...
	case 0x0e000000:
	case 0x0f000000:
		podules_write8((addr >> 24) & 7, 1, addr & 0xffffff, val);
		return;
