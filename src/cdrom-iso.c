...
static void
iso_readsector(uint8_t *b, int sector)
{
	if (iso_empty) {
		return;
	}
...
static int
iso_readtoc(uint8_t *b, uint8_t start_track, int msf)
{
	int len = 4;
	int blocks;
...
static uint8_t
iso_getcurrentsubchannel(uint8_t *b, int msf)
{
	NOT_USED(msf);
...
static void
iso_playaudio(uint32_t pos, uint32_t len)
{
	NOT_USED(pos);
	NOT_USED(len);
