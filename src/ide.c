...
#define WIN_PACKETCMD			0xa0	///< Send a packet command (ATAPI)
...
loadhd(int d, const char *filename)
{
	...
	if (ide.hdfile[d] == NULL) {
		...
		if (ide.hdfile[d] == NULL) {
			...
			if (errno == ENOENT) {
				// Failed because it does not exist, so try to create new file
...
void
writeide(uint16_t addr, uint8_t val)
{
...
atapicommand(void)
{
	...
	case GPCMD_MODE_SENSE_10:
		...
		idebufferb[0] = (uint8_t) (len >> 8);
		idebufferb[1] = (uint8_t) len;
		/* rpclog("ATAPI buffer len %i\n", len);
		for (c = 0; c < len; c++) {
			rpclog("%02x ", idebufferb[c]);
		}
	...
	case GPCMD_START_STOP_UNIT:
		if (idebufferb[4] != 2 && idebufferb[4] != 3 && idebufferb[4] != 0) {
